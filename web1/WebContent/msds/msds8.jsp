...
<%
	...
	strAddr.append("http://msds.kosha.or.kr/openapi/service/msdschem/chemdetail08?ServiceKey=");
	int keyCount = 4;
	strAddr.append(key4);
...
// 	String selectQuery = ("select rownum,rn,chemid from (select rownum rn, chemid from msds_data order by chemid) a where 134869 < chemid");
	String selectQuery = ("select chemid from msds_data_detail where chemid > (select min(chemid) from msds_data_detail where nvl(EXPOSURE_GUIDE,0) = '0')-1 order by chemid");
	...
	try {
		...
		for (int j = 0; j <nameList.size() ; j++) {
			...
			strQuery.append("update msds_data_detail set EXPOSURE_GUIDE = '");
...
// 			System.out.println(url);
			...
			strQuery.append("', KOR_REGULATION='");
			strQuery.append(detailList.get(0).get("1"));
			strQuery.append("', ACGIH_REGULATION='");
			strQuery.append(detailList.get(0).get("2"));
			strQuery.append("', BIOLOGICAL_EXPOSURE_GUIDE='");
			strQuery.append(detailList.get(0).get("3"));
			strQuery.append("', ENGINEERING_MANAGEMENT='");
			strQuery.append(detailList.get(0).get("4"));
			strQuery.append("', PERSONAL_PREVENTION='");
			strQuery.append(detailList.get(0).get("5"));
			strQuery.append("', RESPIRATORY_PROTECTION='");
			strQuery.append(detailList.get(0).get("6"));
			strQuery.append("', EYE_PROTECTION='");
			strQuery.append(detailList.get(0).get("7"));
			strQuery.append("', HAND_PROTECTION='");
			strQuery.append(detailList.get(0).get("8"));
			strQuery.append("', BODY_PROTECTION='");
