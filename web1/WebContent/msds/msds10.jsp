...
<%
	...
	strAddr.append("http://msds.kosha.or.kr/openapi/service/msdschem/chemdetail10?ServiceKey=");
	int keyCount = 1;
	strAddr.append(key1);
...
// 	String selectQuery = ("select rownum,rn,chemid from (select rownum rn, chemid from msds_data order by chemid) a where 134869 < chemid");
	String selectQuery = ("select chemid from msds_data_detail where chemid > (select min(chemid) from msds_data_detail where nvl(STABILITY_HARMFUL_POTENTIAL,0) = '0')-1 order by chemid");
	...
	try {
		...
		for (int j = 0; j <nameList.size() ; j++) {
			...
			strQuery.append("update msds_data_detail set STABILITY_HARMFUL_POTENTIAL = '");
...
// 			System.out.println(url);
			...
			strQuery.append("', AVOID_CONDITION='");
			strQuery.append(detailList.get(0).get("1"));
			strQuery.append("', AVOID_MATERIAL='");
			strQuery.append(detailList.get(0).get("2"));
			strQuery.append("', DECOMPOSITION_HARMFUL='");
