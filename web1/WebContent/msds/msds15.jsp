...
<%
	...
	strAddr.append("http://msds.kosha.or.kr/openapi/service/msdschem/chemdetail15?ServiceKey=");
	int keyCount = 1;
	strAddr.append(key1);
...
// 	String selectQuery = ("select rownum,rn,chemid from (select rownum rn, chemid from msds_data order by chemid) a where 134869 < chemid");
	String selectQuery = ("select chemid from msds_data_detail where chemid > (select min(chemid) from msds_data_detail where nvl(REGULATION_OCCUPATION_SAFE,0) = '0')-1 order by chemid");
	...
	try {
		...
		for (int j = 0; j <nameList.size() ; j++) {
			...
			strQuery.append("update msds_data_detail set REGULATION_OCCUPATION_SAFE = '");
...
// 			System.out.println(url);
			...
			strQuery.append("', REGULATION_HARMFUL_CHEM='");
			strQuery.append(detailList.get(0).get("1"));
			strQuery.append("', REGULATION_DANGEROUS='");
			strQuery.append(detailList.get(0).get("2"));
			strQuery.append("', REGULATION_WASTE='");
			strQuery.append(detailList.get(0).get("3"));
			strQuery.append("', REGULATION_OTHERS='");
			strQuery.append(detailList.get(0).get("4"));
			strQuery.append("', REGULATION_KOR='");
			strQuery.append(detailList.get(0).get("5"));
			strQuery.append("', PERSISTENT_ORGANIC_POLLUTANT='");
			strQuery.append(detailList.get(0).get("6"));
			strQuery.append("', REGULATION_FOREIGN_COUNTRY='");
			strQuery.append(detailList.get(0).get("7"));
			strQuery.append("', US_REGULATION_OSHA='");
			strQuery.append(detailList.get(0).get("8"));
			strQuery.append("', US_REGULATION_CERCLA='");
			strQuery.append(detailList.get(0).get("9"));
			strQuery.append("', US_REGULATION_EPCRA302='");
			strQuery.append(detailList.get(0).get("10"));
			strQuery.append("', US_REGULATION_EPCRA304='");
			strQuery.append(detailList.get(0).get("11"));
			strQuery.append("', US_REGULATION_EPCRA313='");
			strQuery.append(detailList.get(0).get("12"));
			strQuery.append("', US_MANAGE_ROTTERDAM='");
			strQuery.append(detailList.get(0).get("13"));
			strQuery.append("', US_MANAGE_STOCKHOLM='");
			strQuery.append(detailList.get(0).get("14"));
			strQuery.append("', US_MANAGE_MONTREAL='");
			strQuery.append(detailList.get(0).get("15"));
			strQuery.append("', EU_CLASSIFICATION_DECIDE='");
			strQuery.append(detailList.get(0).get("16"));
			strQuery.append("', EU_CLASSIFICATION_DANGER='");
			strQuery.append(detailList.get(0).get("17"));
			strQuery.append("', EU_CLASSIFICATION_SAFETY='");
