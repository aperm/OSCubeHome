...
<%
	...
	strAddr.append("http://msds.kosha.or.kr/openapi/service/msdschem/chemdetail16?ServiceKey=");
	int keyCount = 1;
	strAddr.append(key1);
...
// 	String selectQuery = ("select chemid from msds_data_detail where chemid > (select min(chemid) from msds_data_detail where nvl(MATERIAL_SOURCE,0) = '0') order by chemid");
	String selectQuery = ("select chemid from msds_data_detail where chemid > 018270 order by chemid");
	...
	try {
		...
		for (int j = 0; j <nameList.size() ; j++) {
			...
			strQuery.append("update msds_data_detail set MATERIAL_SOURCE = '");
...
// 			System.out.println(url);
			...
			strQuery.append("', FIRST_WRITE_DATE='");
			strQuery.append(detailList.get(0).get("1"));
			strQuery.append("', REFORM_NUMBER_LAST_DATE='");
			strQuery.append(detailList.get(0).get("2"));
			strQuery.append("', REFORM_NUMBER='");
			strQuery.append(detailList.get(0).get("3"));
			strQuery.append("', REFORM_LATE_DATE='");
...
//	alter table msds_data_detail modify(MATERIAL_SOURCE varchar(3000))
%>
