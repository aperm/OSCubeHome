...
<%
	...
	strAddr.append("http://msds.kosha.or.kr/openapi/service/msdschem/chemdetail06?ServiceKey=");
	int keyCount = 3;
	strAddr.append(key3);
...
// 	String selectQuery = ("select rownum,rn,chemid from (select rownum rn, chemid from msds_data order by chemid) a where 134869 < chemid");
	String selectQuery = ("select chemid from msds_data_detail where chemid > (select min(chemid) from msds_data_detail where nvl(HUMAN_PROTECT_ACTION_PROTEC,0) = '0')-1 order by chemid");
	...
	try {
		...
		for (int j = 0; j <nameList.size() ; j++) {
			...
			strQuery.append("update msds_data_detail set HUMAN_PROTECT_ACTION_PROTEC = '");
...
// 			System.out.println(url);
			...
			strQuery.append("', ENVIRON_PROTECT_ACTION='");
			strQuery.append(detailList.get(0).get("1"));
			strQuery.append("', PURIFICATION_METHOD='");
...
//	alter table msds_data_detail modify(SAFETY_STORAGE_WAY varchar(2500))
%>
