...
<%
	...
	strAddr.append("http://msds.kosha.or.kr/openapi/service/msdschem/chemdetail11?ServiceKey=");
	int keyCount = 1;
	strAddr.append(key1);
...
// 	String selectQuery = ("select rownum,rn,chemid from (select rownum rn, chemid from msds_data order by chemid) a where 134869 < chemid");
	String selectQuery = ("select chemid from msds_data_detail where chemid > (select min(chemid) from msds_data_detail where nvl(HIGH_POSSIBILITY_EXPOSURE,0) = '0')-1 order by chemid");
	...
	try {
		...
		for (int j = 0; j <nameList.size() ; j++) {
			...
			strQuery.append("update msds_data_detail set HIGH_POSSIBILITY_EXPOSURE = '");
...
// 			System.out.println(url);
			...
			strQuery.append("', HEALTH_HAZARD_INFO='");
			strQuery.append(detailList.get(0).get("1"));
			strQuery.append("', ACUTE_TOXICITY='");
			strQuery.append(detailList.get(0).get("2"));
			strQuery.append("', ORAL='");
			strQuery.append(detailList.get(0).get("3"));
			strQuery.append("', DERMAL='");
			strQuery.append(detailList.get(0).get("4"));
			strQuery.append("', INHALATION='");
			strQuery.append(detailList.get(0).get("5"));
			strQuery.append("', SKIN_CORROSIVITY='");
			strQuery.append(detailList.get(0).get("6"));
			strQuery.append("', EYE_DAMAGE='");
			strQuery.append(detailList.get(0).get("7"));
			strQuery.append("', RESPIRATORY_SENSITIZATION='");
			strQuery.append(detailList.get(0).get("8"));
			strQuery.append("', SKIN_SENSITIZATION='");
			strQuery.append(detailList.get(0).get("9"));
			strQuery.append("', CARCINOGENICITY='");
			strQuery.append(detailList.get(0).get("10"));
			strQuery.append("', OCCUPATIONSAFETY_HEALTHACT='");
			strQuery.append(detailList.get(0).get("11"));
			strQuery.append("', MOEL_NOTIFICATION='");
			strQuery.append(detailList.get(0).get("12"));
			strQuery.append("', IARC='");
			strQuery.append(detailList.get(0).get("13"));
			strQuery.append("', OSHA='");
			strQuery.append(detailList.get(0).get("14"));
			strQuery.append("', ACGIH='");
			strQuery.append(detailList.get(0).get("15"));
			strQuery.append("', NTP='");
			strQuery.append(detailList.get(0).get("16"));
			strQuery.append("', UPCLP='");
			strQuery.append(detailList.get(0).get("17"));
			strQuery.append("', GERM_CELL_MUTAGENICITY='");
			strQuery.append(detailList.get(0).get("18"));
			strQuery.append("', REPRODUCTION_TOXICITY='");
			strQuery.append(detailList.get(0).get("19"));
			strQuery.append("', SPECIFIC_ORGAN_TOXICITY_O='");
			strQuery.append(detailList.get(0).get("20"));
			strQuery.append("', SPECIFIC_ORGAN_TOXICITY_M='");
			strQuery.append(detailList.get(0).get("21"));
			strQuery.append("', SUCTION_HARMFUL='");
			strQuery.append(detailList.get(0).get("22"));
