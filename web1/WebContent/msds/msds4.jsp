...
<%
	...
	strAddr.append("http://msds.kosha.or.kr/openapi/service/msdschem/chemdetail04?ServiceKey=");
	int keyCount = 1;
	strAddr.append(key1);
...
// 	String selectQuery = ("select rownum,rn,chemid from (select rownum rn, chemid from msds_data order by chemid) a where 134869 < chemid");
	String selectQuery = ("select chemid from msds_data_detail where chemid > (select min(chemid) from msds_data_detail where nvl(CASE_ENTERED_EYE,0) = '0')-1 order by chemid");
	...
	try {
		...
		for (int j = 0; j <nameList.size() ; j++) {
			...
			strQuery.append("update msds_data_detail set CASE_ENTERED_EYE = '");
...
// 			System.out.println(url);
			...
			strQuery.append("', CASE_CONTACT_SKIN='");
			strQuery.append(detailList.get(0).get("1"));
			strQuery.append("', CASE_INHALE='");
			strQuery.append(detailList.get(0).get("2"));
			strQuery.append("', CASE_EAT='");
			strQuery.append(detailList.get(0).get("3"));
			strQuery.append("', CASE_OTHER_PRECAUTIONS='");
