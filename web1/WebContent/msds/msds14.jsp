...
<%
	...
	strAddr.append("http://msds.kosha.or.kr/openapi/service/msdschem/chemdetail14?ServiceKey=");
	int keyCount = 1;
	strAddr.append(key1);
...
// 	String selectQuery = ("select rownum,rn,chemid from (select rownum rn, chemid from msds_data order by chemid) a where 134869 < chemid");
	String selectQuery = ("select chemid from msds_data_detail where chemid > (select min(chemid) from msds_data_detail where nvl(UN_NUMBER,0) = '0')-1 order by chemid");
	...
	try {
		...
		for (int j = 0; j <nameList.size() ; j++) {
			...
			strQuery.append("update msds_data_detail set UN_NUMBER = '");
...
// 			System.out.println(url);
			...
			strQuery.append("', MODERATE_NAME='");
			strQuery.append(detailList.get(0).get("1"));
			strQuery.append("', TRANSPORT_RISK_RANK='");
			strQuery.append(detailList.get(0).get("2"));
			strQuery.append("', CONTAINER_RANK='");
			strQuery.append(detailList.get(0).get("3"));
			strQuery.append("', MARINE_POLLUTANT='");
			strQuery.append(detailList.get(0).get("4"));
			strQuery.append("', SAFETY_MEASURE='");
			strQuery.append(detailList.get(0).get("5"));
			strQuery.append("', SPILL_EMERGENCY_ACTION='");
