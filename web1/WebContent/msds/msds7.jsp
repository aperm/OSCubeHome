...
<%
	...
	strAddr.append("http://msds.kosha.or.kr/openapi/service/msdschem/chemdetail07?ServiceKey=");
	int keyCount = 6;
	strAddr.append(key6);
...
// 	String selectQuery = ("select rownum,rn,chemid from (select rownum rn, chemid from msds_data order by chemid) a where 134869 < chemid");
	String selectQuery = ("select chemid from msds_data_detail where chemid > (select min(chemid) from msds_data_detail where nvl(SAFETY_HANDEL_KNOWHOW,0) = '0')-1 order by chemid");
	...
	try {
		...
		for (int j = 0; j <nameList.size() ; j++) {
			...
			strQuery.append("update msds_data_detail set SAFETY_HANDEL_KNOWHOW = '");
...
// 			System.out.println(url);
			...
			strQuery.append("', SAFETY_STORAGE_WAY='");
...
//	alter table msds_data_detail modify(SAFETY_HANDEL_KNOWHOW varchar(2000))
%>
