...
<%
	...
	strAddr.append("http://msds.kosha.or.kr/openapi/service/msdschem/chemdetail09?ServiceKey=");
	int keyCount = 2;
	strAddr.append(key2);
...
// 	String selectQuery = ("select rownum,rn,chemid from (select rownum rn, chemid from msds_data order by chemid) a where 134869 < chemid");
	String selectQuery = ("select chemid from msds_data_detail where chemid > (select min(chemid) from msds_data_detail where nvl(APPEARANCE,0) = '0')-1 order by chemid");
	...
	try {
		...
		for (int j = 0; j <nameList.size() ; j++) {
			...
			strQuery.append("update msds_data_detail set APPEARANCE = '");
...
//	alter table msds_data_detail modify(REPRODUCTION_TOXICITY varchar(2000))
%>
