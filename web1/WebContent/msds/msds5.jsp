...
<%
	...
	strAddr.append("http://msds.kosha.or.kr/openapi/service/msdschem/chemdetail05?ServiceKey=");
	int keyCount = 4;
	strAddr.append(key4);
...
// 	String selectQuery = ("select rownum,rn,chemid from (select rownum rn, chemid from msds_data order by chemid) a where 134869 < chemid");
	String selectQuery = ("select chemid from msds_data_detail where chemid > (select min(chemid) from msds_data_detail where nvl(EXTINGUISHING_MATERIAL,0) = '0')-1 order by chemid");
	...
	try {
		...
		for (int j = 0; j <nameList.size() ; j++) {
			...
			strQuery.append("update msds_data_detail set EXTINGUISHING_MATERIAL = '");
...
// 			System.out.println(url);
			...
			strQuery.append("', ADDITIONAL_HAZARD='");
			strQuery.append(detailList.get(0).get("1"));
			strQuery.append("', FIRE_PROTECTION_PREVENTION='");
